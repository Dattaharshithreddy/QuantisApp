...
export default function BacktestScreen() {
  ...
  const filteredAssets = useMemo(() => {
    ...
    return allAssets.filter(a => a.symbol.toUpperCase().includes(q) || (a.name ?? '').toUpperCase().includes(q));
  }, [allAssets, symbolSearch]);
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: T.bg0 }}>
      <ScrollView contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', backgroundColor: T.bg1 ?? T.bg0,
          borderWidth: 1, borderColor: symbolSearch ? T.accent : T.border,
          borderRadius: 8, paddingHorizontal: 10, marginBottom: 8 }}>
          <Text style={{ color: T.textDim, fontSize: 13, marginRight: 6 }}>🔍</Text>
          <TextInput
            value={symbolSearch}
            ...
            style={{ flex: 1, color: T.text, fontSize: 13, paddingVertical: 8 }}
          />
          {symbolSearch.length > 0 && (
            <TouchableOpacity onPress={() => setSymbolSearch('')}>
              <Text style={{ color: T.textDim, fontSize: 16, paddingLeft: 6 }}>✕</Text>
            </TouchableOpacity>
          )}
        </View>
        <Text style={{ color: T.textDim, fontSize: 9, marginBottom: 4 }}>
          {filteredAssets.length} result{filteredAssets.length !== 1 ? 's' : ''}
          {symbolSearch ? ` for "${symbolSearch.toUpperCase()}"` : ''}
          {' · Selected: '}<Text style={{ color: T.accent, fontWeight: '700' }}>{symbol}</Text>
        </Text>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 12 }}>
          <View style={{ flexDirection: 'row', gap: 6 }}>
            {(filteredAssets.length ? filteredAssets : allAssets).map(a => (
              <Pill key={a.symbol + a.src} label={a.symbol} color={T.blue}
