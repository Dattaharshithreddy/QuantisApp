...
export default function MultiChartScreen({ navigation }: any) {
  ...
  const loadAll = useCallback(async () => {
    ...
    for (const sym of slots) {
      ...
      if (!asset) continue;
      try {
        if (asset.src === 'binance' && asset.bnSym) { next[sym] = await fetchBnKlines(asset.bnSym, '15m'); status[sym] = 'live'; }
        else if (asset.src === 'ao' && aoSession?.jwtToken && asset.aoToken && asset.aoEx) { next[sym] = await aoCandles(asset.aoToken, asset.aoEx, '15m', aoSession); status[sym] = 'live'; }
