...
export default function AIChatScreen({ route, navigation }: any) {
  ...
  const buildContext = useCallback(async () => {
    ...
    try {
      ...
      if (asset.src === 'binance' && asset.bnSym) { candles = await fetchBnKlines(asset.bnSym, '15m'); srcLabel = 'Live Binance'; }
      else if (asset.src === 'ao' && aoSession?.jwtToken && asset.aoToken && asset.aoEx) { candles = await aoCandles(asset.aoToken, asset.aoEx, '15m', aoSession); srcLabel = 'Live Angel One'; }
      else if (asset.src === 'av' && asset.avSym && avKey) { candles = await fetchAVKlines(asset.avSym, '15m', avKey); srcLabel = 'Live Alpha Vantage'; }
