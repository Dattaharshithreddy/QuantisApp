...
import { getPortfolio, resetPortfolio, setMode, computePortfolioValue, PaperPortfolioState, PaperPosition } from '../utils/paperPortfolio';
...
export default function PaperTradingScreen({ navigation, route }: any) {
  ...
  const load = useCallback(async () => { setPortfolioState(await getPortfolio()); setTodayPnL(await getTodayPnL()); }, []);
  useEffect(() => { load(); }, [load]);
  ...
  async function handleManualConfirm(accept: boolean) {
    ...
    if (accept) {
      ...
      const result = await attemptOpenPosition(pendingSignal.symbol, route?.params?.timeframe || '15m', pendingSignal.prediction, currentPrice, candles, asset?.type || 'CRYPTO');
              ...
              <Card theme={T}>
                ...
                {p.tradeQuality && (
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginTop: 8 }}>
                    ...
                    <Text style={{ color: T.textDim, fontSize: 9 }}>Grade {p.tradeQuality.grade} · Conf {p.aiConfidence.toFixed(0)}</Text>
                  </View>
                )}
