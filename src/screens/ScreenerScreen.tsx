...
export default function ScreenerScreen() {
  ...
  const runScreener = useCallback(async () => {
    ...
    for (const a of scannable) {
      try {
        if (a.src === 'binance' && a.bnSym) { candleMap[a.symbol] = await fetchBnKlines(a.bnSym, '1h'); scanSet.push(a); }
        else if (a.src === 'ao' && aoSession?.jwtToken && a.aoToken && a.aoEx) { candleMap[a.symbol] = await aoCandles(a.aoToken, a.aoEx, '1h', aoSession); scanSet.push(a); }
        else if (a.src === 'av' && a.avSym && avKey) { candleMap[a.symbol] = await fetchAVKlines(a.avSym, '1h', avKey); scanSet.push(a); }
        else skipped++;
      } catch (_) {
        skipped++;
      }
