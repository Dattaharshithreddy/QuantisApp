...
export default function CorrelationScreen() {
  ...
  const compute = useCallback(async () => {
    ...
    for (const sym of selected) {
      ...
      try {
        if (asset.src === 'binance' && asset.bnSym) candleMap[sym] = await fetchBnKlines(asset.bnSym, '1h');
        else if (asset.src === 'ao' && aoSession?.jwtToken && asset.aoToken && asset.aoEx) candleMap[sym] = await aoCandles(asset.aoToken, asset.aoEx, '1h', aoSession);
