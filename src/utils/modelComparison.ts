...
// Extended entry that also carries the raw trades — used by regime evaluation
// to group model performance per regime without re-running the simulation.
export type ModelComparisonEntryWithTrades = ModelComparisonEntry & { trades: ExecTrade[] };
...
// Used exclusively by regimeEvaluation.ts — existing compareModels is unchanged.
export function compareModelsWithTrades(fitted: FittedEnsemble, execConfig: ExecConfig & { buyThreshold: number }): ModelComparisonEntryWithTrades[] {
  const variants: { name: ModelComparisonEntry['modelName']; pick: (idx: number) => boolean }[] = [
    { name: 'Neural Network',       pick: (idx) => fitted.predictProb(idx).mlpProb > execConfig.buyThreshold },
    { name: 'Logistic Regression',  pick: (idx) => fitted.predictProb(idx).lrProb > execConfig.buyThreshold },
    { name: 'Ensemble',             pick: (idx) => { const p = fitted.predictProb(idx); return p.ensembleProb > execConfig.buyThreshold && p.agree; } },
  ...
  return variants.map(v => {
    ...
    return { modelName: v.name, metrics: computeMetrics(trades, equityCurve, execConfig.startingCapital), trades };
  });
}
