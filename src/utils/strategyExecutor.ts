...
export function simulateSignalStrategy(
  ...
  for (let k = 0; k < walkIndices.length; k++) {
    ...
    if (inPosition) {
      ...
      // Unified execution: intracandle OHLC fill via executionEngine (single implementation)
      const fill = evaluateIntracandleFill(direction, stopLoss, takeProfit, bar, barsHeld, DEFAULT_EXECUTION_CONFIG);

      let exitPrice: number | null = null, exitReason: ExitReason | null = null;
      if (fill.triggered) {
        // applyExitSlippage equivalent — keep slippage in the same direction as paper trading
        const slippedFill = fill.fillType === 'STOP'
          ? (direction === 'LONG' ? fill.actualFill * (1 - config.slippagePct / 100) : fill.actualFill * (1 + config.slippagePct / 100))
          : (direction === 'LONG' ? fill.actualFill * (1 - config.slippagePct / 100) : fill.actualFill * (1 + config.slippagePct / 100));
        exitPrice = slippedFill; exitReason = fill.fillType === 'STOP' ? 'STOP_LOSS' : 'TAKE_PROFIT';
...
export function simulateAIStrategyWithDiagnostics(
  ...
  for (let k = 0; k < walkIndices.length; k++) {
    ...
    if (inPosition) {
      const barsHeld = idx - entryIdx;
      const timedOut = barsHeld >= config.maxHoldingBars;
      // Unified execution via executionEngine (same as paper trading and backtest path 1)
      const fillD = evaluateIntracandleFill(direction, stopLoss, takeProfit, bar, barsHeld, DEFAULT_EXECUTION_CONFIG);

      let exitPrice: number | null = null, exitReason: ExitReason | null = null;
      if (fillD.triggered) {
        const sf = fillD.fillType === 'STOP'
          ? (direction === 'LONG' ? fillD.actualFill * (1 - config.slippagePct / 100) : fillD.actualFill * (1 + config.slippagePct / 100))
          : (direction === 'LONG' ? fillD.actualFill * (1 - config.slippagePct / 100) : fillD.actualFill * (1 + config.slippagePct / 100));
        exitPrice = sf; exitReason = fillD.fillType === 'STOP' ? 'STOP_LOSS' : 'TAKE_PROFIT';
      } else if (timedOut) { exitPrice = bar.close; exitReason = 'TIMEOUT'; }
      else if (k === walkIndices.length - 1) { exitPrice = bar.close; exitReason = 'END_OF_DATA'; }
