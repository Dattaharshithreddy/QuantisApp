...
export function fitEnsemble(candles: Candle[], trainSplitPct: number, seed: number, horizon: number = PRIMARY_HORIZON): FittedEnsemble | null {
  ...
  for (let e = 0; e < 100; e++) mlp.trainEpoch(normTrainX, trainY, 0.08);
  const lr = new LogisticRegression(trainX[0].length, rng);
  for (let e = 0; e < 100; e++) lr.trainEpoch(normTrainX, trainY, 0.15);
  ...
  return {
    predictProb: (idx: number) => {
      ...
      return { mlpProb, lrProb, ensembleProb: (mlpProb + lrProb) / 2, agree: (mlpProb > 0.5) === (lrProb > 0.5) };
    },
