...
export function macd(c: Candle[], fast = 12, slow = 26, signalP = 9) {
  const cl = closes(c);
...
export function rsiSeries(c: Candle[], period = 14): (number | null)[] {
  const cl = closes(c);
...
export function roc(c: Candle[], period = 10): (number | null)[] {
  const cl = closes(c);
  return cl.map((v, i) => i < period ? null : ((v - cl[i - period]) / cl[i - period]) * 100);
}

export function momentum(c: Candle[], period = 10): (number | null)[] {
  const cl = closes(c);
...
// True Strength Index
export function tsi(c: Candle[], longP = 25, shortP = 13): (number | null)[] {
  const cl = closes(c);
...
export function bollinger(c: Candle[], period = 20, mult = 2) {
  const cl = closes(c);
...
export function historicalVolatility(c: Candle[], period = 20): (number | null)[] {
  const cl = closes(c);
