...
// ── Live trading credentials (added for live trading integration) ────────────
// Stored separately from data-only credentials — trading keys have order
// placement permission and must never be confused with read-only data keys.
// binanceApiKey + binanceApiSecret: Binance Spot trading API keys.
//   IMPORTANT: always generate with 'Enable Withdrawals' DISABLED on Binance.
// aoTradingPin: Angel One trading PIN (4–6 digit, different from login password)
//   Used to confirm order placement via SmartAPI.

const LIVE_TRADING_KEYS = [
  'binanceApiKey',
  'binanceApiSecret',
  'aoTradingPin',
] as const;

export type LiveTradingCredentialKey = typeof LIVE_TRADING_KEYS[number];

export async function getLiveTradingCredential(key: LiveTradingCredentialKey): Promise<string | null> {
  try {
    return await SecureStore.getItemAsync(key);
  } catch (e: any) {
    logger.error('secureCredentials', `Failed to read live trading key '${key}': ${e.message}`);
    return null;
  }
}

export async function setLiveTradingCredential(key: LiveTradingCredentialKey, value: string): Promise<void> {
  try {
    await SecureStore.setItemAsync(key, value);
  } catch (e: any) {
    logger.error('secureCredentials', `Failed to write live trading key '${key}': ${e.message}`);
  }
}

export async function deleteLiveTradingCredential(key: LiveTradingCredentialKey): Promise<void> {
  try {
    await SecureStore.deleteItemAsync(key);
  } catch (e: any) {
    logger.error('secureCredentials', `Failed to delete live trading key '${key}': ${e.message}`);
  }
}
