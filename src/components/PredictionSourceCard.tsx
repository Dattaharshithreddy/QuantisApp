...
import { MLPrediction, ARCHITECTURE_VERSION, NEW_CANDLES_THRESHOLD } from '../utils/mlSignal';
...
export function PredictionSourceCard({ prediction, symbol, timeframe, theme: T }: {
  ...
  return (
    <View style={{ backgroundColor: T.card, borderRadius: RADIUS.md, padding: SPACING.md, marginBottom: 10, borderWidth: 1, borderColor: T.cardBorder, ...T.elev1 }}>
      ...
      <Row label="Model Trained On" value={`${prediction.samplesAtActiveModelTraining} samples`} T={T} />
      ...
      <Row label="Last Training Time" value={new Date(prediction.trainedAt).toLocaleString()} T={T} />

      <GroupLabel T={T}>PERFORMANCE</GroupLabel>
      <Row label="Walk-Forward Accuracy" value={prediction.walkForwardAccuracy >= 0 ? `${prediction.walkForwardAccuracy.toFixed(1)}%` : 'n/a'} T={T} />
