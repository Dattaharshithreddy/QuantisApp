...
export function OrderBookCard({
  ...
  // Smart spread formatting: when absolute spread is tiny (common for crypto),
  // show the percentage as the primary value — always human-readable regardless
  // of price magnitude. The absolute value is shown smaller underneath.
  const fmtSpread = (s: number): string => {
    if (s === 0) return '0';
    if (s >= 0.01) return fmt(s);               // normal: 0.60, 0.05, etc.
    if (s >= 0.0001) return s.toFixed(6);       // small: 0.000120
    return s.toExponential(2);                  // tiny: 1.2e-8
  };
    ...
    <View>
      ...
      <View style={{ flexDirection: 'row', gap: 8, marginBottom: 12 }}>
        ...
        <View style={{ flex: 1, backgroundColor: T.bg3, borderRadius: RADIUS.sm, padding: 8, alignItems: 'center' }}>
          ...
          {/* Primary: percentage — always readable regardless of price magnitude */}
          <Text style={{ color: T.text, fontSize: 13, fontWeight: '800', marginTop: 2 }}>
            {spreadPercent != null ? spreadPercent.toFixed(3) + '%' : spread != null ? fmtSpread(spread) : 'n/a'}
          </Text>
          {/* Secondary: absolute value in compact form */}
          {spread != null && (
            <Text style={{ color: T.textDim, fontSize: 8, marginTop: 1 }}>{fmtSpread(spread)}</Text>
          )}
        </View>
