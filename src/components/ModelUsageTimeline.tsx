...
// already exist.
export function ModelUsageTimeline({ prediction, theme: T, overallConfidence }: { prediction: MLPrediction; theme: Theme; overallConfidence?: number }) {
    ...
    <View style={{ marginBottom: 10 }}>
      ...
      {expanded && (
        <View style={{ backgroundColor: T.card, borderRadius: RADIUS.md, padding: SPACING.md, borderWidth: 1, borderColor: T.cardBorder, ...T.elev1 }}>
          ...
          <Step icon="🧠" label={`Model v${prediction.modelVersion ?? '?'}`} detail={`Architecture v${ARCHITECTURE_VERSION}, Training Run #${prediction.trainingRunNumber ?? '?'}`} T={T} />
          ...
          <Step icon="🎯" label="Prediction Generated" detail={`${prediction.action} · ${(overallConfidence ?? prediction.confidence).toFixed(0)}/100 confidence`} T={T} accent={T.blue} last />
        </View>
      )}
    </View>
  );
